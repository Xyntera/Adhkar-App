...
import {
  ...
  Moon, Sun, ArrowLeft, Globe, X, BookMarked, Mic2, BookOpen
} from 'lucide-react';
...
const GlobalStyles = () => (
  <style>{`
    ...
    *, *::before, *::after { box-sizing: border-box; }
    ...
    .verse-active { animation: versePulse 1.8s ease-in-out infinite; }
...
// Full surah audio — Idrees Abkar (quranicaudio.com confirmed working)
const surahAudioUrl = (id) =>
  `https://download.quranicaudio.com/quran/idrees_abkar/opus/${pad3(id)}.opus`;

// QDC timing API — Alafasy reciter (id=7) used as verse-timing reference
// We scale timestamps to match Idrees Abkar's actual duration at runtime
const QDC_TIMING = (chapterId) =>
  `https://api.qurancdn.com/api/qdc/audio/reciters/7/audio_files?chapter=${chapterId}&segments=true`;
...
const WaveBars = ({ color = '#f59e0b', size = 16 }) => (
  <span style={{ display:'inline-flex', alignItems:'flex-end', gap:'2px', height:size }}>
    {[1,2,3,4,5].map(i => (
      <span key={i} className="wave-bar" style={{ height:'100%', background:color }} />
    ))}
  </span>
);
...
const SkeletonVerse = ({ dark }) => (
  <div style={{ padding:'20px', borderBottom:`1px solid ${dark?'#374151':'#f1f5f9'}` }}>
    <div className={dark?'dark-shimmer':'shimmer'} style={{ height:8, borderRadius:4, width:'15%', marginLeft:'auto', marginBottom:14 }} />
    <div className={dark?'dark-shimmer':'shimmer'} style={{ height:30, borderRadius:6, width:'85%', marginLeft:'auto', marginBottom:8 }} />
    <div className={dark?'dark-shimmer':'shimmer'} style={{ height:30, borderRadius:6, width:'65%', marginLeft:'auto', marginBottom:12 }} />
    <div className={dark?'dark-shimmer':'shimmer'} style={{ height:13, borderRadius:4, width:'90%', marginBottom:6 }} />
    <div className={dark?'dark-shimmer':'shimmer'} style={{ height:13, borderRadius:4, width:'60%' }} />
  </div>
);
...
export default function QuranApp() {
  ...
  const timingsRef  = useRef([]); // scaled verse timings [{verse_key, from_ms, to_ms}]
  const refDurRef   = useRef(0);  // Alafasy reference duration (ms)
  const lastIdxRef  = useRef(-1); // prevent redundant scroll

  const [playing, setPlaying]       = useState(false);
  const [activeKey, setActiveKey]   = useState('');   // "1:3" etc.
  ...
  const openSurah = useCallback(async (s) => {
    ...
    try {
      // Fetch verse text + verse timings in parallel
      const [verseRes, timingRes] = await Promise.all([
        fetch(`${QURAN_API}/verses/by_chapter/${s.id}?language=en&words=false&translations=131&per_page=300&fields=text_uthmani`),
        fetch(QDC_TIMING(s.id)),
      ]);
      ...
      // Store verse text
      const list = (verseData.verses || []).map(v => ({
        key:    v.verse_key,
      ...
      // Store reference timing data (Alafasy durations)
      const af = timingData.audio_files?.[0];
      if (af && af.verse_timings?.length) {
        refDurRef.current = af.duration; // total ms for Alafasy
        // Store raw timings — will scale once we know Idrees Abkar's actual duration
        timingsRef.current = af.verse_timings.map(vt => ({
          key:    vt.verse_key,
  ...
  // ── Load & play audio ──────────────────────────────────────────────────────
  const startAudio = useCallback((s) => {
    if (!audioRef.current) return;
    const url = surahAudioUrl(s.id);
    audioRef.current.src = url;
    audioRef.current.load();
    audioRef.current.play()
      .then(() => setPlaying(true))
      .catch(e => { console.error('Play failed:', e); setPlaying(false); });
  }, []);
  ...
  // ── When audio metadata loads → scale timings to Idrees Abkar's duration ──
  const handleMetadata = useCallback(() => {
    if (!audioRef.current) return;
    const idreesDur = audioRef.current.duration * 1000; // convert to ms
    setTotalMs(idreesDur);

    if (refDurRef.current > 0 && timingsRef.current.length > 0) {
      const scale = idreesDur / refDurRef.current;
      // Apply scale factor — Idrees Abkar proportional timestamps
      timingsRef.current = timingsRef.current.map(vt => ({
        key:  vt.key,
        from: vt.from * scale,
        to:   vt.to   * scale,
      }));
      setTimingsReady(true);
    }
  }, []);

  // ── On timeupdate → find current verse by timestamp ────────────────────────
  const handleTimeUpdate = useCallback(() => {
    if (!audioRef.current || timingsRef.current.length === 0) return;
    const ms = audioRef.current.currentTime * 1000;
    setCurrentMs(ms);

    // Binary search for current verse
    const timings = timingsRef.current;
    let lo = 0, hi = timings.length - 1, found = -1;
    while (lo <= hi) {
      const mid = (lo + hi) >> 1;
      if (ms >= timings[mid].from && ms < timings[mid].to) { found = mid; break; }
      else if (ms < timings[mid].from) hi = mid - 1;
      else lo = mid + 1;
    }
    // Fallback: last verse if at end
    if (found === -1 && ms >= timings[timings.length - 1].from) {
      found = timings.length - 1;
    }

    if (found !== -1) {
      const vKey = timings[found].key;
      setActiveKey(prev => {
        if (prev !== vKey) return vKey;
        return prev;
      });

      // Auto-scroll — only when verse changes
      if (found !== lastIdxRef.current) {
        lastIdxRef.current = found;
        const el = verseRefs.current[found];
  ...
  const seek = (e) => {
    if (!audioRef.current || !totalMs) return;
    const pct = parseFloat(e.target.value) / 100;
    audioRef.current.currentTime = (pct * totalMs) / 1000;
  };
  ...
  const goHome = () => {
    ...
    document.title = "القرآن الكريم — إدريس أبكر";
  };

  // ── Derived values ─────────────────────────────────────────────────────────
  const progressPct = totalMs > 0 ? (currentMs / totalMs) * 100 : 0;
  const activeVerseNum = activeKey ? parseInt(activeKey.split(':')[1]) : 0;
  const totalVerses = surah ? surah.v : 0;
  const surahPct = totalVerses > 0 && activeVerseNum > 0
    ? Math.round((activeVerseNum / totalVerses) * 100) : 0;
  ...
  const hdrBg  = dark ? 'rgba(15,23,42,0.96)' : 'rgba(253,248,240,0.96)';
  ...
  return (
    <div style={{ minHeight:'100vh', background:bg, color:text, fontFamily:'Inter,sans-serif', transition:'background .3s,color .3s' }}>
      ...
      <audio
        ...
        onEnded={() => { setPlaying(false); setActiveKey(''); setCurrentMs(0); }}
      />

      {/* ══ HEADER ══════════════════════════════════════════════════════════ */}
      <header style={{
        position:'sticky', top:0, zIndex:50,
        background:hdrBg, backdropFilter:'blur(12px)',
        borderBottom:`1px solid ${border}`,
      }}>
        <div style={{ maxWidth:860, margin:'0 auto', padding:'0 16px', height:60, display:'flex', alignItems:'center', gap:10 }}>

          {view === 'surah' ? (
            <button onClick={goHome} style={{ background:'none', border:'none', cursor:'pointer', color:text, display:'flex', padding:6 }}>
              <ArrowLeft size={20} />
            </button>
          ) : (
            <div style={{ display:'flex', alignItems:'center', gap:10 }}>
              <div style={{ width:38, height:38, borderRadius:12, background:'linear-gradient(135deg,#059669,#0d9488)', display:'flex', alignItems:'center', justifyContent:'center', boxShadow:'0 2px 8px rgba(5,150,105,.35)' }}>
          ...
          {view === 'surah' && surah && (
            <div style={{ flex:1, lineHeight:1.25 }}>
              <div style={{ fontWeight:700, fontSize:17, fontFamily:'Scheherazade New,serif', direction:'rtl' }}>{surah.ar}</div>
          ...
          <div style={{ marginLeft:'auto', display:'flex', alignItems:'center', gap:6 }}>
            {view === 'home' && (
              <div style={{ display:'flex', alignItems:'center', gap:8, background:cardBg, border:`1px solid ${border}`, borderRadius:24, padding:'7px 14px', boxShadow:'0 1px 4px rgba(0,0,0,.06)' }}>
                ...
                />
                {search && <button onClick={() => setSearch('')} style={{ background:'none', border:'none', cursor:'pointer', color:muted_, display:'flex' }}><X size={13} /></button>}
              </div>
            )}
            ...
            {view === 'surah' && (
              <>
                <button
                  ...
                  style={{ background: showTrans ? (dark?'rgba(5,150,105,.2)':'#d1fae5') : 'transparent', border:`1px solid ${showTrans?'#059669':border}`, color:showTrans?'#059669':muted_, borderRadius:8, padding:'5px 10px', cursor:'pointer', display:'flex', alignItems:'center', gap:4, fontSize:12, fontWeight:600 }}
                >
                  <Globe size={13}/> EN
                </button>
                <button onClick={() => toggleBm(surah.id)} style={{ background:'none', border:'none', cursor:'pointer', color:bookmarks.includes(surah.id)?'#f59e0b':muted_, display:'flex', padding:6 }}>
                  <BookMarked size={18} />
                </button>
              </>
            )}

            <button onClick={() => setDark(p => !p)} style={{ background:'none', border:'none', cursor:'pointer', color:muted_, display:'flex', padding:6 }}>
      ...
      {view === 'home' && (
        <main style={{ maxWidth:860, margin:'0 auto', padding:'0 16px 80px' }}>
          {/* Hero */}
          <div style={{ margin:'24px 0 20px', borderRadius:24, background:'linear-gradient(135deg,#065f46,#0f766e,#064e3b)', padding:'28px 24px', color:'#fff', position:'relative', overflow:'hidden', boxShadow:'0 12px 40px rgba(6,95,70,.3)' }}>
            <div style={{ position:'absolute', top:-30, right:-30, width:160, height:160, background:'rgba(255,255,255,.07)', borderRadius:'50%' }} />
            <div style={{ position:'relative' }}>
              <span style={{ display:'inline-block', background:'#f59e0b', color:'#78350f', fontSize:11, fontWeight:700, padding:'3px 10px', borderRadius:20, letterSpacing:.5, textTransform:'uppercase', marginBottom:14 }}>★ Featured Reciter</span>
              <div style={{ display:'flex', alignItems:'flex-start', justifyContent:'space-between' }}>
                <div>
                  ...
                  <div style={{ display:'flex', gap:8, flexWrap:'wrap' }}>
                    {['Live Ayah Sync','Wave Animation','Full 114 Surahs'].map(t => (
                      <span key={t} style={{ background:'rgba(255,255,255,.15)', fontSize:12, padding:'4px 12px', borderRadius:20 }}>{t}</span>
                    ))}
                  </div>
                </div>
          ...
          <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:14 }}>
            ...
            {bookmarks.length > 0 && <div style={{ fontSize:12, color:muted_ }}>{bookmarks.length} bookmarked</div>}
          </div>

          <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(300px,1fr))', gap:10 }}>
            {filtered.map(s => {
              const grads = ['linear-gradient(135deg,#059669,#0d9488)','linear-gradient(135deg,#d97706,#ea580c)','linear-gradient(135deg,#2563eb,#7c3aed)','linear-gradient(135deg,#db2777,#e11d48)'];
              const g = grads[Math.floor((s.id-1)/30) % grads.length];
              return (
                <button key={s.id} onClick={() => openSurah(s)}
                  style={{ display:'flex', alignItems:'center', gap:12, background:cardBg, border:`1px solid ${border}`, borderRadius:16, padding:'14px 16px', cursor:'pointer', textAlign:'left', color:text, transition:'box-shadow .15s' }}
                  onMouseEnter={e=>e.currentTarget.style.boxShadow='0 4px 20px rgba(0,0,0,.12)'}
                  onMouseLeave={e=>e.currentTarget.style.boxShadow='none'}
                >
                  <div style={{ width:40, height:40, borderRadius:12, background:g, display:'flex', alignItems:'center', justifyContent:'center', color:'#fff', fontWeight:700, fontSize:13, flexShrink:0, boxShadow:'0 2px 8px rgba(0,0,0,.2)' }}>{s.id}</div>
                  ...
                  <div style={{ fontFamily:'Scheherazade New,serif', fontSize:20, lineHeight:1.3, flexShrink:0 }}>{s.ar}</div>
                </button>
              );
            })}
          </div>
      ...
      {/* ══ SURAH VIEW ══════════════════════════════════════════════════════ */}
      {view === 'surah' && surah && (
        <main style={{ maxWidth:780, margin:'0 auto', padding:'0 16px 220px' }}>
          {/* Surah banner */}
          <div style={{ margin:'20px 0 14px', borderRadius:20, background:'linear-gradient(135deg,#065f46,#0f766e)', padding:'22px 24px', color:'#fff', textAlign:'center' }}>
            ...
            <div style={{ fontSize:12, opacity:.6, marginTop:4 }}>{surah.v} verses · {surah.t} · Juz {surah.j}</div>

            {/* Surah completion bar */}
            {activeKey && (
              <div style={{ marginTop:14 }}>
                <div style={{ background:'rgba(255,255,255,.2)', borderRadius:4, height:5, overflow:'hidden' }}>
                  <div style={{ width:`${surahPct}%`, background:'#f59e0b', height:'100%', borderRadius:4, transition:'width .5s ease' }} />
                </div>
                <div style={{ fontSize:11, opacity:.7, marginTop:5 }}>
                  Verse {activeVerseNum} of {totalVerses} · {surahPct}% complete
                </div>
              </div>
            )}
          </div>
          ...
          {/* Bismillah */}
          {surah.id !== 9 && (
            <div style={{ textAlign:'center', fontFamily:'Amiri Quran,serif', fontSize:26, lineHeight:2, color:dark?'#fcd34d':'#065f46', padding:'8px 0 4px' }}>
              {BISMILLAH}
            </div>
          )}
                ...
                return (
                  <div
                    ...
                    style={{
                      ...
                      background: isActive ? (dark?'rgba(234,179,8,.1)':'rgba(254,243,199,.7)') : 'transparent',
                      borderLeft: `4px solid ${isActive ? '#f59e0b' : 'transparent'}`,
                  ...
                  >
                        ...
                        {isActive && (
                          <div style={{ display:'flex', alignItems:'center', gap:6 }}>
                            <span style={{ fontSize:11, fontWeight:700, color:'#f59e0b', textTransform:'uppercase', letterSpacing:.5 }}>
                              {playing ? '▶ Now Reciting' : '⏸ Paused'}
                            </span>
                            {playing && <WaveBars color="#f59e0b" size={12} />}
                          </div>
                        )}
                      </div>
                      <span style={{ fontSize:11, color:muted_, fontWeight:500 }}>{v.key}</span>
                    </div>
                    ...
                    <div
                      ...
                      style={{
                        ...
                        lineHeight:2.2,
                        textAlign:'right',
                        color: isActive ? (dark?'#fde68a':'#064e3b') : text,
                        marginBottom:10,
                    ...
                    {/* Translation */}
                    {showTrans && v.trans && (
                      <div style={{ fontSize:14, lineHeight:1.7, color:muted_, borderTop:`1px solid ${border}`, paddingTop:10 }}>
                        {v.trans}
                      </div>
                    )}
                  </div>
      ...
      {view === 'surah' && surah && (
        <div style={{
          ...
          background:hdrBg, backdropFilter:'blur(16px)',
          borderTop:`1px solid ${border}`,
          boxShadow:'0 -4px 30px rgba(0,0,0,.15)',
        }}>
          {/* Overall surah progress */}
          <div style={{ height:4, background:dark?'#334155':'#e2e8f0' }}>
            <div style={{ width:`${progressPct}%`, height:'100%', background:'linear-gradient(90deg,#059669,#f59e0b)', transition:'width .1s linear' }} />
          </div>

          <div style={{ maxWidth:780, margin:'0 auto', padding:'12px 16px 14px' }}>
            {/* Now playing info */}
            <div style={{ display:'flex', alignItems:'center', gap:10, marginBottom:10 }}>
              <div style={{ width:38, height:38, borderRadius:10, background:'linear-gradient(135deg,#059669,#0d9488)', display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0 }}>
                {playing ? <WaveBars color="#fff" size={14}/> : <Mic2 size={17} color="#fff"/>}
              </div>
              <div style={{ flex:1, minWidth:0 }}>
                <div style={{ fontWeight:600, fontSize:13, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>
                  {activeKey
                    ? <>Verse {activeVerseNum} of {totalVerses} — <span style={{ fontFamily:'Scheherazade New,serif' }}>{surah.ar}</span></>
                    : <>{surah.name} — <span style={{ fontFamily:'Scheherazade New,serif' }}>{surah.ar}</span></>
                  }
                </div>
                <div style={{ fontSize:11, color:playing?'#059669':muted_, fontWeight:playing?600:400, marginTop:1 }}>
                  إدريس أبكر · {playing ? 'Now Reciting' : activeKey ? 'Paused' : 'Tap ▶ to begin'}
                  {totalMs > 0 && <span style={{ color:muted_, fontWeight:400, marginLeft:8 }}>{fmt(currentMs)} / {fmt(totalMs)}</span>}
                </div>
              </div>
              {/* Timing status indicator */}
              {!timingsReady && playing && (
                <div style={{ fontSize:10, color:'#f59e0b', background:dark?'rgba(234,179,8,.15)':'rgba(254,243,199,1)', border:'1px solid #f59e0b', borderRadius:6, padding:'3px 8px', flexShrink:0 }}>
                  Syncing…
                </div>
              )}
            </div>
            ...
            <div style={{ display:'flex', alignItems:'center', gap:10 }}>
              ...
              <button
                ...
                style={{ background:'none', border:'none', cursor:'pointer', color:muted_, display:'flex', flexDirection:'column', alignItems:'center', fontSize:10, gap:1, padding:'4px 6px' }}
              >
                <SkipBack size={18}/> <span style={{ fontSize:9 }}>10s</span>
              </button>

              {/* Play / Pause */}
              <button
                onClick={togglePlay}
                style={{ width:54, height:54, borderRadius:'50%', background:'linear-gradient(135deg,#059669,#0d9488)', border:'none', cursor:'pointer', display:'flex', alignItems:'center', justifyContent:'center', boxShadow:'0 4px 16px rgba(5,150,105,.4)', color:'#fff', transition:'transform .1s', flexShrink:0 }}
              ...
              <button
                ...
                style={{ background:'none', border:'none', cursor:'pointer', color:muted_, display:'flex', flexDirection:'column', alignItems:'center', fontSize:10, gap:1, padding:'4px 6px' }}
              >
                <SkipForward size={18}/> <span style={{ fontSize:9 }}>10s</span>
              </button>

              <div style={{ flex:1 }}/>

              {/* Volume */}
              <button onClick={() => setMuted(p=>!p)} style={{ background:'none', border:'none', cursor:'pointer', color:muted?'#ef4444':muted_, display:'flex', padding:6 }}>
                {muted ? <VolumeX size={18}/> : <Volume2 size={18}/>}
              </button>
              <input
                type="range" min={0} max={1} step={0.05} value={muted?0:volume}
                onChange={e=>{ setVolume(parseFloat(e.target.value)); setMuted(false); }}
                style={{ width:70, height:4, accentColor:'#059669', cursor:'pointer' }}
              />
            </div>
          </div>
        </div>
